...
namespace CitiFine.Areas.Identity.Pages.Account
{
    ...
    {
        private readonly SignInManager<CitiFineUser> _signInManager;
        private readonly UserManager<CitiFineUser> _userManager;
        private readonly IUserStore<CitiFineUser> _userStore;
        ...
        private CitiFineUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<CitiFineUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(CitiFineUser)}'. " +
                    $"Ensure that '{nameof(CitiFineUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<CitiFineUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<CitiFineUser>)_userStore;
        }
    }
}
